...
;//ƒV[ƒ“–¼	Fwx
;//file–¼	F1190(END)
;//“oêl•¨	FålŒöA@ˆê˜YAˆ©—œA‰Ä“sArˆä
;//•‘•		F§•
;//“ú•t		F8/19
;//ŠÔ		F()
;//”wŒi		F()
;//—\‘z—e—Ê	F‘S‘Ì‚ğ’Ê‚µ‚Ä10K‘OŒã
;//”õl		FålŒöˆêlÌ‹“_
;//QQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQ
*1190_TOP
;{SceneSet “ì‚Ì“‡}

;//m:‚±‚Ìƒtƒ@ƒCƒ‹‚ªˆ©—œƒ‹[ƒg‚ÌÅIƒuƒƒbƒN
;//m:D‚ª‚¿‚å‚Á‚ÆTW‘½‚¢‚Æ‚¢‚¤‚Ì‚Å‚Ì‚ÅŒ¸‚ç‚µ‚Ü‚µ‚½

[bgm storage="BGM19"]
;//ôbgm19@‚Ó‚½‚è‚Á‚«‚èBˆ£DŒnB
[se buf=0 storage="seE012" loop=true]
;//’ª‘›
;•s—vH[wait_c time=1500]

[bg storage="sky01D"][trans_c cross time=2000]
...
[ChrSetEx layer=5 chbase="ak2_se2"][ChrSetParts layer=5 chface="F2_ak01"][ChrSetXY layer=5 x=0 y=0][trans_c cross time=1000]
;•s—vH[wait_c time=1000]
[ChrSetEx layer=5 chbase="ak2_se2"][ChrSetParts layer=5 chface="F2_ak03"][ChrSetXY layer=5 x=0 y=0][trans_c cross time=1000]
;•s—vH[wait_c time=1000]
[sysbt_meswin]
...
;//ˆ©—œ‘å—§•iƒnƒ`ƒ}ƒL‚È‚µj@•\î02@ÎŠç‚P
...
;//Ÿ‚¨˜b‚Ì’Ê‚èA“K“–‚È•ªŠò‚ğ“ü‚ê‚Ä‚¨‚«‚Ü‚µ‚½Bƒtƒ‰ƒO‚Íİ’è‚³‚ê‚Ä‚¢‚é‚à‚Ì‚ğ—¬—p‚µ‚Ä‚¢‚Ü‚·

;//ğŒ•ªŠò
;//ƒtƒ‰ƒOcall_kaduo¬—§‚µ‚Ä‚¢‚é‚©‚Ç‚¤‚©
;//YES¨_ƒ‰ƒxƒ‹end_akariA‚Öjump
;//NO¨_ƒ‰ƒxƒ‹end_akariB‚Öjump
[if exp="f.l_call_kaduo==1"][jump target=*end_akariA][endif]
[jump target=*end_akariB]
...
[ChrSetEx layer=5 chbase="na1_sdf_a"][ChrSetParts layer=5 chface="F1_na03"][ChrSetXY layer=5 x=300 y=0]
...
[fadeoutbgm time=500]
;<SoundFade 1,2000>
;//<SoundLoop 1,ON><SoundRun 1,Stop>
[stopse buf=0]
;<SoundFade 2,OUT,2000>
;//<SoundLoop 2,OFF><SoundRun 2,Stop>
[stopse buf=1]
...
*end01
;//ƒUƒbƒsƒ“ƒOŠJ•ú’m@ˆê“xŒ©‚½‚çÄ¶‚³‚ê‚È‚¢‚ªƒUƒbƒvŠJ•úƒtƒ‰ƒO‚ğ•ÏX‚µ‚½ê‡‚Í—vC³
[if exp="sf.g_clear==1"][jump target=*end02][endif]
[movie storage="zap_open.mpg"]
*end02
;//•Ï” "g_end_akariA" ‚ª¬—§‚Í ‰ÁZ‚µ‚È‚¢B
[if exp="sf.g_end_akariA==1"][jump target=*NO_KASAN][endif]
;//ƒNƒŠƒA‰ñ”‰ÁZ
...
[movie storage="Prolog_skip.mpg"]
*end03
;//ƒUƒbƒsƒ“ƒOŠJ•ú’m@9070 9080ŠJ•ú
[if exp="sf.g_end_akari==1"][jump target=*end04][endif]
[movie storage="zap_open.mpg"]
*end04
;//•Ï” "g_end_akariB" ‚ª¬—§‚Í ‰ÁZ‚µ‚È‚¢B
[if exp="sf.g_end_akariB==1"][jump target=*NO_KASAN2][endif]
;//ƒNƒŠƒA‰ñ”‰ÁZ
[eval exp="sf.g_clear_time = sf.g_clear_time + 1"]
*NO_KASAN2
;//ƒ^ƒCƒgƒ‹‰æ–ÊØ‚è‘Ö‚¦—pƒtƒ‰ƒOBƒNƒŠƒA‚µ‚½ƒLƒƒƒ‰‚ğƒIƒ“A‘¼‚ğƒIƒt‚É‚·‚é
